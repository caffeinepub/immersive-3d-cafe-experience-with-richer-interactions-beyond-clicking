 ...
 * Features subtle idle animation (tail wag, head bob, breathing).
...
export default function DogCharacter({
  ...
  useFrame((state) => {
    ...
    // Tail wag
    if (tailRef.current) {
      tailRef.current.rotation.z = Math.sin(time * 3) * 0.4;
    }

    // Head bob
    if (headRef.current) {
      headRef.current.position.y = 0.18 + Math.sin(time * 2) * 0.01;
    }
